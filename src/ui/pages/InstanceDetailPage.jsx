...
module.exports = createReactClass({
  ...
  renderWithData: function() {
    ...
    const renderSsh = (ssh) => iconLink(`ssh ${ssh.fqdn}`, (this.copyToClipboard `ssh ${ssh.fqdn}`), Icons.Base.Copy)
    ...
    if(this.props.instanceLink){
      titleComponent = <Anchor reverse={true} href={this.props.instanceLink} target='_blank'
        icon={<Icons.Base.Link style={{width:60, marginBottom:20}} />}
        label={<Heading tag='h1' style={{display:'inline-block'}} strong={true}>{this.props.title}</Heading>} />
    }
    ...
    return (
      <Split flex='left' priority='left'>
        <DetailPage title={titleComponent} >
          ...
          <Section pad='medium'>
            <List>
              ...
              {this.props.instance.storageBucket &&
                li('Storage bucket', iconLink(this.props.instance.storageBucket, this.props.onOpenBucketPage))
              }
          ...
          <div className='terminal-wrapper'>
            <Terminal outputStyle={{minHeight: 100, maxHeight: 250}}>
              ...
              <pre>
                {this.props.instance.logs && this.props.instance.logs.startup.join('')}
              </pre>
              ...
              {this.props.instance.logs && this.props.instance.logs &&
                <kbd>docker-compose down</kbd>
              }
              {this.props.instance.logs && this.props.instance.logs &&
                <pre>
                  {this.props.instance.logs.teardown.join('')}
                </pre>
              }
            </Terminal>
          </div>
          ...
          {_.map(this.props.instance.services, (service, name) =>
              <Section key={name} pad='medium'>
                <Header style={{minHeight:0, margin:"0px 20px 0px 0px"}}>
                  <Anchor reverse={true} href={this.props.serviceLinks[name]} target='_blank'
                  ...
                  <Box flex={true} justify='end' direction='row'>
                    <Anchor reverse={true} target='_blank'
                      path={`/instances/${this.props.instance.name}/${name}/logs`}
                      icon={<Icons.Base.DocumentTxt style={{width:20}} />}
                      label={<span style={{fontSize:20, fontWeight:'normal'}}>Logs</span>} />
                  </Box>
                </Header>
                <List>
                  {service.container && li('Created', moment(service.container.created).fromNow())}
                  {li('State', renderStatus(service))}
                  {li('FQDN', service.fqdn)}
                  {service.container && li('Container name', service.container.name)}
        ...
        <Sidebar size='medium' colorIndex='light-2' direction='column'>
          ...
          <InstanceControls instanceName={this.props.instance.name} onStopInstance={this.props.onStopInstance}/>
        </Sidebar>
      </Split>
    )
  }
