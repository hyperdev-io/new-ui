...
const { Article, Button, Box, Header, Heading, Search, Title, Split, Sidebar, Paragraph, Icons, Menu, List, ListItem } = require('grommet');
const FilterControl = require('grommet-addons/components/FilterControl');

module.exports = function ({ title, searchValue, totalResults, items = [], selectedIdx, onSearch, onClearSearch, renderItem, onListItemSelected }) {
  const _onListItemSelected = idx => typeof onListItemSelected === 'function' ? onListItemSelected(items != null ? items[idx] : undefined) : undefined;
  ...
  return (
    <Article>
      ...
      <List selectable={true} onSelect={_onListItemSelected} selected={selectedIdx || -1}>
