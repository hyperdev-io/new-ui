...
const { Box, Button, Heading, Header, Footer, Form, FormFields, FormField, TextInput, Select, CheckBox, Anchor, Icons } = require('grommet');
...
module.exports  = createReactClass({
  ...
  appToOptionValue: (app) => ({
    ...
    _.filter(this.props.apps, (app) => (app.name.match(this.props.appsearch) || app.version.match(this.props.appsearch)))
    },

  startInstance: function(evt){ this.props.onStartInstance()},
    ...
    <fieldset>
      ...
      {params.map((param) =>
        <FormField key={param} label={param} htmlFor={param} size='medium'>
          <TextInput value={params ? [`param_${param}`] : ''} onDOMChange={onParamChanged(param)} />
        </FormField>
      )}
    </fieldset>,
  render: function(){
            ...
            <fieldset>
              <FormField label='Application definition' htmlFor='app' size='medium'>
                <Select placeHolder='Search'
                  ...
                  options={(this.getAppOptions() || this.props.apps).map(this.appToOptionValue)}
                  value={`${this.props.selectedAppName} (${this.props.selectedAppVersion})`}
                  onChange={this.onAppSelectChange} />
              </FormField>
              <FormField label='Instance name' htmlFor='name' size='medium' error="">
                <TextInput placeHolder='A unique name to identify this instance' value={this.props.name} onDOMChange={this.onNameChange} />
              </FormField>
            </fieldset>
            {this.props.selectedApp && this.renderAppParams(this.getAppParams(), this.onParamChanged)}

            <fieldset>
              <Box direction='row' justify='between'>
                <CheckBox toggle={true} defaultChecked  label={
                  <Heading style={{ paddingTop: 10 }} tag='h3'>Persisted Storage</Heading>
                } />
              </Box>

              <FormField label='Storage bucket' size='medium' >
                <Select onSearch={ ()=> console.log('search')} value={this.props.bucket || this.props.name} onChange={this.onBucketSelected} options={this.props.buckets}/>
              </FormField>
            </fieldset>
          </FormFields>

          <Footer pad={{vertical: 'medium'}}>
            <Button type="button" primary={true} label="Start Instance" onClick={this.startInstance} />
          </Footer>
        </Form>
      </Box>
    )
